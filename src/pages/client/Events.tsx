  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}`, boxShadow: "0 24px 60px rgba(46,32,28,0.18)" }}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h3
              ...
              style={{ color: ALMA.ink, fontSize: "clamp(1.4rem, 2.4vw, 1.8rem)" }}
            >
              {event.title}
            </h3>
          </div>
          <button
            ...
            style={{ border: `1px solid ${ALMA.border}`, color: ALMA.ink, opacity: 0.6 }}
          >
            <X size={15} />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["transfer", "cash"] as const).map((m) => {
                const sel = method === m;
                const tint = m === "transfer" ? ALMA.berry : ALMA.orange;
                return (
                  <button
                    key={m}
                    type="button"
                    ...
                    style={{
                      backgroundColor: sel ? ALMA.blush : "transparent",
                      border: `1px solid ${sel ? tint : ALMA.border}`,
                    }}
                  >
                    <p className="font-bebas text-[1rem] leading-tight" style={{ color: ALMA.ink }}>
                      {m === "transfer" ? "Transferencia" : "Efectivo"}
                    </p>
                    <p className="text-[0.72rem] mt-0.5" style={{ color: ALMA.ink, opacity: 0.6 }}>
...
function PaymentSection({ event, onDone }: { event: ClientEvent; onDone: () => void }) {
  ...
  return (
    <div className="rounded-3xl p-5 sm:p-7 space-y-5" style={{ backgroundColor: ALMA.blush }}>
      <div>
        <span className="text-[0.62rem] font-medium uppercase tracking-[0.24em]" style={{ color: ALMA.berry }}>
          Completa tu pago
        </span>
        <h3 className="font-bebas mt-1 leading-tight" style={{ color: ALMA.ink, fontSize: "clamp(1.3rem, 2vw, 1.7rem)" }}>
        ...
        {(["transfer", "cash"] as const).map((m) => {
          ...
          return (
            <button
              ...
              style={{
                backgroundColor: sel ? ALMA.cream : "transparent",
                border: `1px solid ${sel ? tint : ALMA.border}`,
              }}
            >
              <p className="font-bebas text-[1rem] leading-tight" style={{ color: ALMA.ink }}>
      ...
      {method === "transfer" ? (
        <div className="space-y-4">
          <div className="rounded-2xl p-4" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
            {[
          ...
          <div>
            ...
            {!file ? (
              ...
              >
                <Upload size={18} style={{ margin: "0 auto 6px", color: ALMA.berry, opacity: 0.65 }} />
              ...
              <div className="flex items-center gap-3 rounded-2xl p-3" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.olive}55` }}>
                <span className="grid h-9 w-9 place-items-center rounded-xl shrink-0" style={{ backgroundColor: ALMA.olive, color: ALMA.cream }}>
                  <CheckCircle2 size={15} />
                </span>
                <p className="flex-1 truncate text-[0.88rem]" style={{ color: ALMA.ink }}>
                  {file.name}
                </p>
                <button
                  type="button"
  ...
  const handleAppleWalletAdd = async () => {
    ...
    try {
      ...
      if (contentType.includes("application/vnd.apple.pkpass")) {
        ...
        a.href = url;
        a.download = "alma-event-pass.pkpass";
    ...
    <>
      ...
      <Section>
        <div className="rounded-3xl p-5 sm:p-7" style={{ backgroundColor: ALMA.blush }}>
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-[0.78rem]" style={{ color: ALMA.ink, opacity: 0.7 }}>
                {event.instructor}
              </p>
              {event.description && (
                <p className="mt-2 text-[0.95rem] leading-[1.7]" style={{ color: ALMA.ink, opacity: 0.8 }}>
                  {event.description}
                </p>
              )}
            </div>
          ...
          {event.memberDiscount > 0 && (
            <p className="mt-3 text-[0.86rem]" style={{ color: ALMA.olive }}>
      ...
      <Section title="Cupo">
        <div className="rounded-2xl p-5" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
          ...
          {isFull && !myReg && (
            <p className="mt-3 text-[0.84rem]" style={{ color: ALMA.coral }}>
      ...
      {event.requirements && (
        <Section title="Requisitos">
          <p className="text-[0.95rem] leading-[1.7]" style={{ color: ALMA.ink, opacity: 0.78 }}>
            {event.requirements}
          </p>
        </Section>
      )}
        ...
        <Section title="Tu pase">
          <div className="rounded-3xl p-5 sm:p-7" style={{ backgroundColor: ALMA.blush }}>
            <div className="flex items-center gap-2 mb-4">
              <CheckCircle2 size={16} style={{ color: ALMA.olive }} />
            ...
            {eventPassCode ? (
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                ...
                >
                  <QRCodeSVG value={eventPassCode} size={120} bgColor={ALMA.cream} fgColor={ALMA.ink} />
                </div>
                <div className="min-w-0">
                  <p className="text-[0.62rem] uppercase tracking-[0.22em]" style={{ color: ALMA.berry }}>
                    Código del pase
                  </p>
                  <p className="mt-1 font-mono text-[0.86rem] break-all" style={{ color: ALMA.ink, opacity: 0.78 }}>
                    {eventPassCode}
                  </p>
                  <button
                    type="button"
                  ...
                  <p className="mt-3 text-[0.78rem]" style={{ color: ALMA.ink, opacity: 0.55 }}>
                    Presenta el QR en recepción para tu check-in.
                  </p>
                </div>
              </div>
            ) : (
              <p className="text-[0.86rem]" style={{ color: ALMA.ink, opacity: 0.6 }}>
            ...
            {eventPassCode && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-5">
                <button
                  ...
                  style={{ backgroundColor: ALMA.cream, color: ALMA.ink, border: `1px solid ${ALMA.border}` }}
                >
                  <AppleIcon />
            ...
            <button
              ...
              style={{ color: ALMA.destructive, opacity: 0.85 }}
            >
  ...
  return (
    ...
    >
      ...
      >
        <div className="grid grid-cols-[auto_1fr_auto] items-start gap-4">
          ...
          <div className="min-w-0">
            <h3
              ...
              style={{ color: ALMA.ink, fontSize: "clamp(1.15rem, 1.6vw, 1.4rem)" }}
            >
              {event.title}
            </h3>
            <p className="mt-1 text-[0.78rem]" style={{ color: ALMA.ink, opacity: 0.6 }}>
              {event.instructor} · {typeInfo?.label}
            </p>
          </div>
          <div className="text-right shrink-0">
            <p className="font-bebas leading-none tabular-nums" style={{ color: ALMA.berry, fontSize: "1.15rem" }}>
              {event.price === 0 ? "Gratis" : formatCurrency(currentPrice)}
            </p>
            {event.earlyBirdPrice && calcCurrentPrice(event) === event.earlyBirdPrice && (
              <p className="mt-0.5 text-[0.62rem] uppercase tracking-[0.18em]" style={{ color: ALMA.orange }}>
                Early bird
              </p>
            )}
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1.5 text-[0.78rem]" style={{ color: ALMA.ink, opacity: 0.6 }}>
        ...
        <div className="mt-4">
          <div className="h-1 rounded-full overflow-hidden" style={{ backgroundColor: ALMA.blush }}>
            <div
          ...
          <div className="mt-2 flex items-center justify-between gap-2">
            <span className="text-[0.7rem] flex items-center gap-1.5" style={{ color: ALMA.ink, opacity: 0.55 }}>
              ...
              <Section>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    style={{
                      backgroundColor: filter === "all" ? ALMA.berry : "transparent",
                      color: filter === "all" ? ALMA.cream : ALMA.ink,
                      border: `1px solid ${filter === "all" ? ALMA.berry : ALMA.border}`,
                    }}
                  >
                    Todos
                  </button>
                  {EVENT_TYPES.filter((t) => usedTypes.includes(t.value)).map((t) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          backgroundColor: sel ? ALMA.berry : "transparent",
                          color: sel ? ALMA.cream : ALMA.ink,
                          border: `1px solid ${sel ? ALMA.berry : ALMA.border}`,
                        }}
                      >
