    ...
    <ClientAuthGuard requiredRoles={["client"]}>
      <AppShell hideGreeting>
        <section
          ...
          style={{ backgroundColor: ALMA.blush, border: `1px solid ${ALMA.border}` }}
        >
          <div
            className="absolute -right-20 -top-24 h-72 w-72 rounded-full pointer-events-none"
            style={{ background: `radial-gradient(circle, ${ALMA.coral}26 0%, transparent 68%)` }}
          />
          <div
            className="absolute -bottom-28 left-1/3 h-72 w-72 rounded-full pointer-events-none"
            style={{ background: `radial-gradient(circle, ${ALMA.olive}22 0%, transparent 68%)` }}
          />

          <div className="relative grid gap-7 xl:grid-cols-[1fr_auto] xl:items-end">
            <div>
              <span className="inline-flex items-center gap-2 text-[0.66rem] font-medium uppercase tracking-[0.24em]" style={{ color: ALMA.berry }}>
                <Sparkles size={13} />
                Tu agenda Alma
              </span>
              <h1 className="mt-3 font-bebas leading-[0.95] tracking-tight" style={{ color: ALMA.ink, fontSize: "clamp(2rem, 4.6vw, 4.2rem)" }}>
                Reserva tu clase
                <span className="block italic font-alilato font-normal" style={{ color: ALMA.berry, fontSize: "0.7em" }}>
                  sin perder el ritmo.
                </span>
              </h1>
              <p className="mt-4 max-w-[62ch] text-[0.98rem] leading-[1.7]" style={{ color: ALMA.ink, opacity: 0.68 }}>
            ...
            <div className="flex flex-wrap items-center gap-2 xl:justify-end">
              ...
              <div className="rounded-full px-5 py-3 text-center" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
                <p className="text-[0.58rem] uppercase tracking-[0.24em]" style={{ color: ALMA.berry }}>Semana</p>
                <p className="mt-0.5 font-bebas text-[1.15rem] leading-none tabular-nums" style={{ color: ALMA.ink }}>
                  {weekLabel}
                </p>
              </div>
              <button
        ...
        <section className="mt-5 grid grid-cols-1 gap-3 xl:grid-cols-[1.3fr_0.9fr_0.8fr]">
          {hasActive ? (
            <div className="rounded-[1.5rem] p-4 sm:p-5" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[0.62rem] font-medium uppercase tracking-[0.22em]" style={{ color: ALMA.berry }}>
                  Tu membresía
                </span>
                <Tag tint={CAT_TINT[membershipCat ?? "all"]}>{CAT_LABEL[membershipCat ?? "all"]}</Tag>
              </div>
              <div className="mt-3 flex items-end justify-between gap-4">
                <p className="font-bebas text-[1.45rem] leading-none" style={{ color: ALMA.ink }}>
                  {membership.planName ?? membership.plan_name}
                </p>
                <div className="text-right">
                  <p className="font-bebas text-[2rem] leading-none tabular-nums" style={{ color: ALMA.berry }}>
                    {isUnlimited ? "∞" : classesRemaining}
                  </p>
                  <p className="text-[0.62rem] uppercase tracking-[0.18em]" style={{ color: ALMA.ink, opacity: 0.55 }}>
                    clases por usar
                  </p>
                </div>
              </div>
          ...
          <div className="rounded-[1.5rem] p-4 sm:p-5" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
            <span className="text-[0.62rem] font-medium uppercase tracking-[0.22em]" style={{ color: weeklyCap?.remaining === 0 ? ALMA.coral : ALMA.olive }}>
              Tope semanal
            </span>
            <div className="mt-3 flex items-end justify-between gap-4">
              <p className="text-[0.9rem] leading-[1.55]" style={{ color: ALMA.ink, opacity: 0.72 }}>
                {weeklyCap
              ...
              <p className="font-bebas text-[2rem] leading-none tabular-nums" style={{ color: ALMA.olive }}>
          ...
          <div className="rounded-[1.5rem] p-4 sm:p-5" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
            <span className="text-[0.62rem] font-medium uppercase tracking-[0.22em]" style={{ color: ALMA.coral }}>
              Próxima disponible
            </span>
            <div className="mt-3 flex items-end justify-between gap-4">
              <p className="text-[0.9rem] leading-[1.55]" style={{ color: ALMA.ink, opacity: 0.72 }}>
              ...
              <CalendarDays size={24} style={{ color: ALMA.coral }} />
            </div>
          </div>
        </section>

        {membershipCat && membershipCat !== "all" && membershipCat !== "mixto" && (
          <p className="mt-3 text-[0.84rem]" style={{ color: ALMA.ink, opacity: 0.65 }}>
            Tu membresía permite reservar solo clases de{" "}
            <span style={{ color: ALMA[CAT_TINT[membershipCat]], fontWeight: 600 }}>{CAT_LABEL[membershipCat]}</span>.
          </p>
        )}
        ...
        <section className="mt-8">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              <h2 className="text-[0.7rem] font-medium uppercase tracking-[0.24em]" style={{ color: ALMA.ink, opacity: 0.65 }}>
                Vista semanal
              </h2>
              <p className="mt-1 text-[0.86rem]" style={{ color: ALMA.ink, opacity: 0.55 }}>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span
                ...
                style={{ backgroundColor: `${ALMA.olive}1a`, color: ALMA.olive }}
              >
          ...
          <div className="lg:hidden space-y-3">
            <p className="text-[0.66rem] font-medium uppercase tracking-[0.24em]" style={{ color: ALMA.berry }}>
              Lista móvil
            </p>
            {days.map((day, i) => {
              const dayClasses = classesForDay(day);
    ...
    >
      <div className={compact ? "flex items-start gap-3" : ""}>
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-bebas text-[1.05rem] leading-tight tracking-tight truncate" style={{ color: isBooked ? ALMA.olive : disabled ? ALMA.ink : cls.color }}>
                {cls.name}
              </span>
              {isBooked && <CheckCircle2 size={13} style={{ color: ALMA.olive }} />}
              {locked && <Lock size={12} style={{ color: ALMA.ink, opacity: 0.45 }} />}
            </div>
            <p className="mt-1 flex items-center gap-1.5 text-[0.78rem] tabular-nums" style={{ color: ALMA.ink, opacity: 0.62 }}>
              <Clock3 size={12} />
          ...
          <span
            ...
            style={{
              backgroundColor: isBooked ? `${ALMA.olive}1f` : `${cls.color}18`,
              color: isBooked ? ALMA.olive : cls.color,
            }}
          >
            {statusLabel}
          </span>
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center justify-between gap-2 text-[0.72rem]" style={{ color: ALMA.ink, opacity: 0.62 }}>
          ...
          <div className="h-1.5 overflow-hidden rounded-full" style={{ backgroundColor: `${ALMA.ink}12` }}>
            <span
              className="block h-full rounded-full transition-[width] duration-300"
              style={{ width: `${capacityPct}%`, backgroundColor: full ? ALMA.coral : cls.color }}
            />
          </div>
        </div>
      </div>
...
const DayColumn = ({
  ...
  <article
    ...
    style={{
      backgroundColor: todayMark ? `${ALMA.blush}` : ALMA.cream,
      border: `1px solid ${todayMark ? `${ALMA.berry}33` : ALMA.border}`,
    }}
  >
    <div className="mb-3 flex items-end justify-between gap-2">
      <div>
        <p className="text-[0.62rem] uppercase tracking-[0.2em]" style={{ color: todayMark ? ALMA.berry : ALMA.ink, opacity: todayMark ? 1 : 0.5 }}>
          {DAY_LABELS[dayIndex]}
        </p>
        <p className="font-bebas text-[2rem] leading-none tabular-nums" style={{ color: todayMark ? ALMA.berry : ALMA.ink }}>
          {format(day, "d")}
        </p>
      </div>
      <span className="rounded-full px-2 py-1 text-[0.62rem] font-medium" style={{ backgroundColor: todayMark ? ALMA.cream : ALMA.blush, color: ALMA.berry }}>
        {dayClasses.length}
      </span>
    </div>
    ...
    <div className="space-y-2.5">
      {loadingClasses ? (
        ...
        <div className="rounded-[1rem] border border-dashed p-4 text-center text-[0.78rem]" style={{ color: ALMA.ink, borderColor: ALMA.border, opacity: 0.45 }}>
          Sin clases
        </div>
      ) : (
        dayClasses.map((cls) => (
...
const MobileDay = ({
  ...
  <article className="rounded-[1.5rem] p-4" style={{ backgroundColor: ALMA.cream, border: `1px solid ${ALMA.border}` }}>
    <div className="mb-3 flex items-center justify-between gap-3">
      <div>
        <p className="text-[0.62rem] uppercase tracking-[0.2em]" style={{ color: isToday(day) ? ALMA.berry : ALMA.ink, opacity: isToday(day) ? 1 : 0.55 }}>
          {DAY_LABELS[dayIndex]}
        </p>
        <h3 className="font-bebas text-[1.55rem] leading-none" style={{ color: ALMA.ink }}>
          {format(day, "d 'de' MMMM", { locale: es })}
        </h3>
      </div>
      <span className="rounded-full px-3 py-1 text-[0.7rem] font-medium" style={{ backgroundColor: ALMA.blush, color: ALMA.berry }}>
    ...
    <div className="space-y-2.5">
      {loadingClasses ? (
        ...
        <div className="rounded-[1rem] border border-dashed p-4 text-[0.84rem]" style={{ borderColor: ALMA.border, color: ALMA.ink, opacity: 0.5 }}>
