...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="relative w-full overflow-x-auto overflow-y-hidden overscroll-x-contain">
      <table ref={ref} className={cn("w-full min-w-[640px] sm:min-w-0 caption-bottom text-sm", className)} {...props} />
    </div>
  ),
);
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-line transition-colors data-[state=selected]:bg-sunken hover:bg-canvas", className)}
      {...props}
    />
  ),
);
