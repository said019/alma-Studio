...
/* ── AlmaLoader ────────────────────────────────────────────────────────────
   Loader con identidad Alma: anillo concéntrico animado en berry/coral
   sobre fondo blush. Útil mientras pedimos la URL firmada o esperamos
   metadata del video.
*/
export const AlmaLoader = ({
  ...
  >
    ...
    <div
      ...
      style={{ background: `radial-gradient(circle, ${ALMA.coral} 0%, transparent 70%)` }}
      aria-hidden
    />
    <div
      className="absolute -bottom-32 -left-20 h-72 w-72 rounded-full opacity-30 blur-3xl"
      style={{ background: `radial-gradient(circle, ${ALMA.berry} 0%, transparent 70%)` }}
      aria-hidden
    />
    ...
    <div className="relative h-16 w-16">
      <span
        className="absolute inset-0 rounded-full alma-spin-slow"
        style={{
          background: `conic-gradient(from 0deg, transparent, ${ALMA.berry})`,
          mask: "radial-gradient(circle, transparent 58%, black 60%)",
          WebkitMask: "radial-gradient(circle, transparent 58%, black 60%)",
        }}
      />
      <span
        className="absolute inset-2 rounded-full alma-spin-fast"
        style={{
          background: `conic-gradient(from 180deg, transparent, ${ALMA.coral})`,
    ...
    <div className="relative">
      <p
        ...
        style={{ color: ALMA.ink, fontSize: "clamp(1.05rem, 1.6vw, 1.4rem)" }}
      >
        {label}
      </p>
      {sublabel && (
        <p className="mt-1 font-alilato italic text-[0.92rem]" style={{ color: ALMA.berry, opacity: 0.8 }}>
          {sublabel}
        </p>
      )}
    </div>
    ...
    <style>{`
      ...
      .alma-spin-slow { animation: alma-spin 2.2s linear infinite; }
      .alma-spin-fast { animation: alma-spin 1.3s linear infinite reverse; }
    `}</style>
  </div>
);
...
/* ── AlmaVideoPlayer ───────────────────────────────────────────────────────
   <video> con controles propios alineados al sistema Alma.
   - Scrubber muestra:
     · capa base (border) · buffer cargado (cream translúcido) ·
     · progreso reproducido (gradiente berry → coral) · pulgar al hover/scrub
   - Overlay de buffering con anillo Alma cuando estamos en waiting/seeking
*/
type Props = {
  src: string;
  className?: string;
    ...
    >
      ...
      {waiting && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          ...
          >
            <Loader2 size={22} className="animate-spin" color={ALMA.cream} />
          </span>
        </div>
      )}
      ...
      {!playing && !waiting && (
        ...
        >
          <Play size={26} fill={ALMA.berry} className="ml-1" />
        </button>
      )}
      ...
      >
        ...
        >
          ...
          <span
            ...
            style={{
              left: `${pct}%`,
              backgroundColor: ALMA.cream,
              boxShadow: `0 0 0 2px ${ALMA.berry}`,
        ...
        <div className="mt-2 flex items-center justify-between gap-3 pointer-events-auto">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              {playing ? <Pause size={14} fill={ALMA.berry} /> : <Play size={14} fill={ALMA.berry} className="ml-0.5" />}
            </button>
            ...
            <div className="hidden sm:flex items-center gap-2 group/vol">
              <button
                ...
                style={{ backgroundColor: "rgba(255,247,242,0.14)", color: ALMA.cream }}
              >
              ...
              <input
                ...
                aria-label="Volumen"
                className="alma-volume w-0 group-hover/vol:w-20 transition-[width] duration-300"
              />
            </div>
      ...
      <style>{`
        ...
        .alma-volume::-webkit-slider-thumb {
          -webkit-appearance: none;
          appearance: none;
          height: 12px;
          width: 12px;
          ...
          box-shadow: 0 0 0 2px ${ALMA.coral};
          cursor: pointer;
        }
        .alma-volume::-moz-range-thumb {
          height: 12px;
          width: 12px;
          border: 0;
          border-radius: 999px;
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-3 mb-2">
        ...
        <span className="font-bebas tabular-nums text-[1rem]" style={{ color: ALMA.ink }}>
          {Math.floor(v)}%
        </span>
      </div>
      ...
      >
        <div
          className="absolute inset-y-0 left-0 rounded-full transition-[width] duration-200 ease-out alma-bar-fill"
          style={{
            width: `${v}%`,
            background: `linear-gradient(90deg, ${ALMA.berry} 0%, ${ALMA.coral} 100%)`,
          }}
        />
        {/* Shimmer activo mientras sube y aún no termina */}
        {v > 0 && v < 100 && (
          <div
            className="absolute inset-y-0 left-0 alma-bar-shimmer pointer-events-none"
            style={{
              width: `${v}%`,
              background:
      ...
      {hint && (
        <p className="mt-2 text-[0.78rem]" style={{ color: ALMA.ink, opacity: 0.6 }}>
          {hint}
        </p>
      )}

      <style>{`
        @keyframes alma-shimmer {
          0%   { transform: translateX(-100%); }
          100% { transform: translateX(100%); }
        }
        .alma-bar-shimmer { animation: alma-shimmer 1.4s ease-in-out infinite; }
      `}</style>
    </div>
  );
};
