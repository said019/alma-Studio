...
Expected: exit 0. (La corrida real requiere `DATABASE_URL` de la DB de Alma — se ejecuta en QA, no en CI local. Si está disponible: `DATABASE_URL='...' node endpoint-auditor/audit/db-mutating-test-video-plans.mjs` → `2 passed` y `cascade check ... 0`.)
...
- [ ] **Step 4: QA manual (requiere server + DB de staging/Alma)**
